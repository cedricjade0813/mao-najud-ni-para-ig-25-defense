...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['mark_all_read'])) {
    ...
    error_log("🔴 DEBUG: Student row ID in session: " . (isset($_SESSION['student_row_id']) ? $_SESSION['student_row_id'] : 'NOT SET'));
    ...
    try {
        ...
        if ($student_id && is_numeric($student_id)) {
            ...
            error_log("🔴 DEBUG: Mark all as read - Student ID: $student_id");
            ...
            $count_before = $db->prepare('SELECT COUNT(*) FROM notifications WHERE student_id = ? AND is_read = 0');
            $count_before->execute([$student_id]);
            ...
            $stmt = $db->prepare('UPDATE notifications SET is_read = 1 WHERE student_id = ? AND is_read = 0');
            ...
            $count_after = $db->prepare('SELECT COUNT(*) FROM notifications WHERE student_id = ? AND is_read = 0');
            $count_after->execute([$student_id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['refresh_cache'])) {
    ...
    try {
        if (isset($_SESSION['student_row_id'])) {
            ...
            $countStmt = $db->prepare('SELECT COUNT(*) FROM notifications WHERE student_id = ? AND is_read = 0');
...
@media (max-width: 400px) {
    /* Notifications title and subtitle - matching profile page styling */
    .notifications-title {
...
error_log("🔴 DEBUG: Loading notifications page for student: $student_id");
...
try {
    ...
    $stmt = $fresh_db->prepare('SELECT id, message, is_read, created_at FROM notifications WHERE student_id = ? ORDER BY created_at DESC');
    $stmt->execute([$student_id]);
...
<main class="flex-1 overflow-y-auto bg-gray-50 p-6 ml-16 md:ml-64 mt-[56px] scrollbar-hide">
    ...
    <div class="mb-8">
        <div class="flex items-center">
            ...
            <div>
                <h1 class="text-3xl font-bold text-gray-900 mb-2 notifications-title">Notifications</h1>
                <p class="text-gray-600 notifications-subtitle">Stay updated with your latest activities</p>
            </div>
        </div>
    </div>
    ...
    function filterNotifications() {
     ...
     function markAllAsRead() {
         ...
         const studentId = <?php echo json_encode($_SESSION['student_row_id'] ?? null); ?>;
         console.log('🔴 DEBUG: Student ID from page:', studentId);
         ...
         fetch('notifications.php', {
             ...
             body: 'mark_all_read=1&student_id=' + encodeURIComponent(studentId)
         }).then(response => {
