...
echo "<!-- Faculty ID: " . ($_SESSION['faculty_id'] ?? 'Not set') . " -->";
echo "<!-- Faculty Name: " . ($_SESSION['faculty_name'] ?? 'Not set') . " -->";
...
<main class="flex-1 overflow-y-auto bg-gray-50 p-6 ml-16 md:ml-64 mt-[56px] scrollbar-hide">
    <!-- Page Header -->
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900 mb-2">Medical History</h1>
        <p class="text-gray-600">View and manage patient health records and past consultations.</p>
    </div>
    ...
    function searchMedicalHistory(searchTerm, page = 1) {
        ...
        if (!searchTerm || searchTerm.trim() === '') {
            ...
            fetch('search_medical_history.php', {
                ...
                body: `search=&page=${page}&student_id=<?php echo $faculty_id; ?>`
            })
            .then(response => {
                if (!response.ok) {
        ...
        fetch('search_medical_history.php', {
            ...
            body: `search=${encodeURIComponent(searchTerm)}&page=${page}&student_id=<?php echo $faculty_id; ?>`
        })
        .then(response => {
            if (!response.ok) {
                throw new Error('Network response was not ok');
