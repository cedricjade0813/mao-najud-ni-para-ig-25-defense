...
// Debug: Log the faculty ID for verification
error_log("Logged in Faculty ID: " . $faculty_id);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['appointment_date'])) {
    ...
    if (empty($appointment_date) || empty($appointment_time) || empty($reason) || empty($email)) {
        ...
        if ($existing) {
            ...
            $insert_stmt = $conn->prepare('INSERT INTO appointments (faculty_id, date, time, reason, status, email, parent_email, doctor_id) VALUES (?, ?, ?, ?, "pending", ?, ?, ?)');
            $insert_stmt->bind_param('isssssi', $faculty_id, $appointment_date, $appointment_time, $reason, $email, $parent_email, $doctor_id);

            if ($insert_stmt->execute()) {
                $booking_success = "Appointment booked successfully! You will receive a confirmation email.";
                error_log("Appointment booked successfully for faculty ID: $faculty_id");
            } else {
...
try {
    ...
    $count_queries = [
        'pending' => "SELECT COUNT(*) FROM appointments WHERE faculty_id = ? AND status = 'pending' AND date = ?",
        'approved' => "SELECT COUNT(*) FROM appointments WHERE faculty_id = ? AND status IN ('approved', 'confirmed')",
        'declined' => "SELECT COUNT(*) FROM appointments WHERE faculty_id = ? AND status = 'declined'",
        'rescheduled' => "SELECT COUNT(*) FROM appointments WHERE faculty_id = ? AND status = 'rescheduled' AND (date > ? OR (date = ? AND (
    ...
    foreach ($count_queries as $status => $query) {
        ...
        if ($status === 'pending') {
            $stmt->bind_param('is', $faculty_id, $currentDate);
        } elseif ($status === 'rescheduled') {
            $currentTime = date('H:i:s');
            $stmt->bind_param('issssss', $faculty_id, $currentDate, $currentDate, $currentTime, $currentTime, $currentTime, $currentTime);
        } else {
...
<script>
    ...
    function loadAppointments(status, page = 1, searchTerm = '') {
        ...
        fetch('get_appointments.php', {
                ...
                body: `status=${status}&patient_id=${facultyId}&page=${page}&search=${encodeURIComponent(searchTerm)}`
            })
            .then(response => {
            ...
            .then(data => {
                ...
                console.log('Faculty ID sent:', facultyId);
                ...
                if (data.success && data.appointments.length > 0) {
                    ...
                    data.appointments.forEach(appt => {
                        console.log('Rendering appointment for faculty_id:', appt.faculty_id);
    ...
    function initializeFacultyTab(tabName) {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const activeTab = localStorage.getItem('facultyActiveTab') || 'appointments';
        initializeFacultyTab(activeTab);
        ...
        function renderCalendar(month, year) {
            ...
            for (let d = 1; d <= daysInMonth; d++) {
                ...
                // Add doctor profession text if there's a schedule
                if (doctorSchedule) {
