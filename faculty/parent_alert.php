<?php
// filepath: c:/xampp/htdocs/CMS/faculty/parent_alert.php
...
// Query: count visits per symptom for this faculty in the current week
$alerts = [];
try {
    ...
    $stmt->execute([$faculty_id, $startOfWeek, $endOfWeek]);
...
<main class="flex-1 overflow-y-auto bg-gray-50 p-6 ml-16 md:ml-64 mt-[56px]">
    <h2 class="text-2xl font-bold mb-6 text-gray-800">Faculty Notification Alert</h2>
    <?php if (!empty($alerts)): ?>
        <div class="bg-yellow-100 border-l-4 border-yellow-500 text-yellow-800 p-4 rounded mb-6 flex items-center gap-2">
            <i class="ri-error-warning-line text-2xl"></i>
            <span>You have visited the clinic 3 or more times this week for the same symptom. Please contact your supervisor and let them know about your health status.</span>
        </div>
