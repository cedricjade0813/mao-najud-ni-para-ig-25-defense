...
try {
    ...
    if ($appointment_id) {
        ...
        $stmt = $db->prepare('UPDATE appointments SET status = "cancelled" WHERE id = ? AND faculty_id = ? AND status IN ("pending", "approved") LIMIT 1');
        ...
        $stmt = $db->prepare('UPDATE appointments SET status = "cancelled" WHERE faculty_id = ? AND date = ? AND time = ? AND reason = ? AND status = "pending" LIMIT 1');
